...
INDEX_HTML = r'''
    ...
    <body class="h-screen w-screen overflow-hidden flex flex-col font-sans select-none relative">
        ...
        <header class="h-16 border-b border-slate-900 bg-slate-950/80 backdrop-blur-md flex items-center justify-between px-6 z-10 select-none cursor-default" onmousedown="onHeaderMouseDown(event)">
        ...
        <script>
            ...
            function onHeaderMouseDown(e) {
                // Ensure only left click triggers window dragging and ignore clicks on UI buttons/inputs
                if (e.button !== 0 || e.target.closest('button') || e.target.closest('nav') || e.target.closest('input') || e.target.closest('select')) return;
                isDragging = true;
            ...
            function onResizeHandleMouseMove(e) {
                ...
                if (dx !== 0 || dy !== 0) {
                    ...
                    startResizeY = e.screenY;
                    if (window.pywebview && window.pywebview.api) {
                        window.pywebview.api.resize_window(dx, dy);
                    }
                }
            }
